...
SIM_ROOT = pathlib.Path('/mnt/hgfs/Data/pvec_problems/simulated')
REAL_ROOT = pathlib.Path('/mnt/hgfs/Data/pvec_problems/maastricht')
...
def svb_sim_data(params, var, opts, shape):
    ...
    data += np.random.normal(0, var, size=data.shape)
    return data 
...
def prepare_sim_data():
    ...
    opts = dict(plds=PLDS, repeats=PLD_REPEATS, tau=TAU, casl=True, 
                ...
                pvwm=pvs[...,1])
    data = svb_sim_data([CBF[0], ATT[0], CBF[1], ATT[1]], NOISE_VAR / 2, opts, spc.size)
